...
public class MQSCAutoIndentStrategy extends DefaultIndentLineAutoEditStrategy {
	 ...
	 */
	 protected int findMatchingOpenBracket(IDocument document, int _line, int _end, int closingBracketIncrease) throws BadLocationException {
	 ...
	 */
	 private int getBracketCount(IDocument document, int start, int end, boolean _ignoreCloseBrackets) throws BadLocationException {

	  boolean ignoreCloseBrackets = _ignoreCloseBrackets;
		int begin = start;
		int bracketcount= 0;
		while (begin < end) {
	 ...
	 protected void smartIndentAfterNewLine(IDocument document, DocumentCommand command) {
		...
		try {
	        ...
	        if (MQSCEditorPlugin.getDefault().isDebugging()) {
	            MQSCEditorPlugin.getDefault().getLog().log( new Status(IStatus.ERROR,MQSCEditor.PLUGIN_ID,0, MQSCEditorMessages.getString("AutoIndent.error.bad_location_1"), excp)); 
	        }
		}
	}
	 ...
	 */
	 protected void smartInsertAfterBracket(IDocument document, DocumentCommand command) {
		...
		try {
	        ...
	        if (MQSCEditorPlugin.getDefault().isDebugging()) {
	            MQSCEditorPlugin.getDefault().getLog().log( new Status(IStatus.ERROR,"com.ibm.mq.explorer.ms0s.mqsceditor",0, MQSCEditorMessages.getString("AutoIndent.error.bad_location_1"), excp)); 
	        }
		}
	}
}
