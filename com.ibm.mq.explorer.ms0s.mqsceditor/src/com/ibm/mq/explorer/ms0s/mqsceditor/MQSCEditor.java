...
public class MQSCEditor extends TextEditor {
  ...
  public MQSCEditor() {
    super();
    U.debug("MQSCEditor (constructor)");
  }

  protected void createActions() {
    super.createActions();

    IAction a = new TextOperationAction(MQSCEditorMessages.getResourceBundle(), "ContentAssistProposal.", this, ISourceViewer.CONTENTASSIST_PROPOSALS);
    a.setActionDefinitionId(ITextEditorActionDefinitionIds.CONTENT_ASSIST_PROPOSALS);
    setAction("ContentAssistProposal", a);

    a = new TextOperationAction(MQSCEditorMessages.getResourceBundle(), "ContentAssistTip.", this, ISourceViewer.CONTENTASSIST_CONTEXT_INFORMATION);
    a.setActionDefinitionId(ITextEditorActionDefinitionIds.CONTENT_ASSIST_CONTEXT_INFORMATION);
    setAction("ContentAssistTip", a);
    a = new TextOperationAction(MQSCEditorMessages.getResourceBundle(), "ContentFormatProposal.", this, ISourceViewer.FORMAT);
...
//            fOutlinePage.setInput(null);
  ...
  public void doSetInput(IEditorInput _input) throws CoreException {
    super.doSetInput(_input);
    this.input = _input;
    if (fOutlinePage != null)
      fOutlinePage.setInput(_input);
    validateAndMark();
  }
  ...
  protected void removeMarkers() {
    ...
    try {
      ...
      if (MQSCEditorPlugin.getDefault().isDebugging()) {
        MQSCEditorPlugin.getDefault().getLog().log(new Status(IStatus.ERROR, MQSCEditor.PLUGIN_ID, 0, "Core Exception", e1));
      }
    }
  }
  ...
  protected void validateAndMark() {
    ...
    for (Iterator iter = myDM.keySet().iterator(); iter.hasNext();) {
      ...
      if (el.isInvalid()) {
        for (Iterator invIter = el.getInvalidEvents().iterator(); invIter.hasNext();) {
          MQSCCommandEvent myEvent = (MQSCCommandEvent) invIter.next();
          try {
            columnNumber = myEvent.getOffset();
            lineNumber = document.getLineOfOffset(columnNumber);
            lineStart = document.getLineOffset(lineNumber);
            map.clear();
          }
          catch (BadLocationException e) {
            if (MQSCEditorPlugin.getDefault().isDebugging()) {
              MQSCEditorPlugin.getDefault().getLog().log(new Status(IStatus.ERROR, MQSCEditor.PLUGIN_ID, 0, "Bad Location Exception", e));
            }
          }
          ...
          Integer charEnd = new Integer(columnNumber + myEvent.getLength());
          if (charEnd != null)
            map.put(IMarker.CHAR_END, charEnd);

          map.put(IMarker.SEVERITY, new Integer(IMarker.SEVERITY_ERROR));
          if (myEvent.getType() == MQSCCommandEvent.INVALID_EVENT) {
            message = "MQSC syntax error: invalid token \"" + myEvent.getValue() + "\" on line " + lineNumber;
          }
          else if (myEvent.getType() == MQSCCommandEvent.BAD_PAREN_EVENT) {
            message = "MQSC syntax error: Unmatched parentheses  \"" + myEvent.getValue() + "\" at character " + (columnNumber - lineStart) + " on line "
                + lineNumber;

          }
          else if (myEvent.getType() == MQSCCommandEvent.BAD_QUOTE_EVENT) {
            message = "MQSC syntax error: Unmatched single-quote \"" + myEvent.getValue() + "\" on line " + lineNumber;
          }
          ...
          try {
            ...
            if (MQSCEditorPlugin.getDefault().isDebugging()) {
              MQSCEditorPlugin.getDefault().getLog().log(new Status(IStatus.ERROR, MQSCEditor.PLUGIN_ID, 0, "Core Exception", ee));
            }
          }
        }
      }
    ...
    ITypedRegion[] partitions = extension3.getDocumentPartitioner(MQSCEditorPlugin.MQSC_PARTITIONING).computePartitioning(0, document.getLength());
    for (int i = 0; i < partitions.length; i++) {
      ...
      if (partType.equalsIgnoreCase(IDocument.DEFAULT_CONTENT_TYPE)) {
        try {
          ...
          partitionText = document.get(partitions[i].getOffset(), partitions[i].getLength());
        ...
        Integer charEnd = new Integer(offset + partitions[i].getLength() - 1);
        if (charEnd != null)
          map.put(IMarker.CHAR_END, charEnd);

        map.put(IMarker.SEVERITY, new Integer(IMarker.SEVERITY_ERROR));
        message = "MQSC syntax error: invalid text starting with \"" + partitionText.substring(0, (length > 10) ? 10 : length) + "\" on line " + lineNumber;
        MarkerUtilities.setMessage(map, message);
        map.put(IMarker.MESSAGE, message);
  ...
  public Object getAdapter(Class required) {
    if (IContentOutlinePage.class.equals(required)) {
      if (fOutlinePage == null) {
        fOutlinePage = new MQSCContentOutlinePage(getDocumentProvider(), this);
  ...
  protected void initializeEditor() {
    ...
    U.debug("MQSCEditor initializeEditor()");
