        ...
        case '\r':
            if (currentState != INVALID_STATE) {
                ...
                return generateToken(fDefaultToken,
                        MQSCCommandEvent.TERMINAL_EVENT, "", mqscScanner
                                .getOffset(), partStart, partLength, partIndex);
            }
            break;
        case ' ':
        case '\u0009':
        ...
        case '-':
            ...
            if (lookAhead == '\r') {
                ...
                return generateToken(fInvalidObjectToken,
                        MQSCCommandEvent.INVALID_EVENT, eventVal, mqscScanner
                                .getOffset() - 1, partStart, partLength, partIndex);
            }
            break;
        case ')':
            parenCount--;
            ...
            if (fColumn == UNDEFINED) {
                {
                    ...
                    case INITIAL_STATE:
                        ...
                        if (token != null) {
                            ...
                            //return Token.UNDEFINED;
                            return generateToken(fInvalidObjectToken,
                                    MQSCCommandEvent.INVALID_EVENT, fBuffer
                                            .toString(), startPos, partStart,
                                    partLength, partIndex);
                        }
                    case FOUND_COMMAND_STATE:
                        ...
                        if (objList != null) {
                            ...
                            return generateToken(fValidObjectToken,
                                    ...
                                    objectType, startPos, partStart, partLength, partIndex);
                        } else {
                            currentState = INVALID_STATE;
                    ...
                    case FOUND_OBJECT_NAME_STATE:
                        ...
                        if (closeParen == ')') {
                            ...
                            return generateToken(fDefaultToken,
                                    ...
                                    objectName, startPos, partStart, partLength, partIndex);
                        } else {
                            currentState = INVALID_STATE;
...
//        char c;
        ...
        try {
            ...
            if (myToken instanceof MQSCToken) {
                myEvent = new MQSCCommandEvent(this, EventType, EventValue,
                        startPos, tokLength, partitionStart, partitionLength, partitionIndex);
  ...
  //      String EventValue = myEvent.getValue().trim();
        switch (myEvent.getType()) {
        //This doesn't work right.
        /*
         * case MQSCCommandEvent.OBJECT_NAME_EVENT: if (unbalancedQuote) { if
         * (EventValue.startsWith("'") && !EventValue.endsWith("'")) { myToken =
         * fInvalidObjectToken;
         * myEvent.setEventType(MQSCCommandEvent.BAD_QUOTE_EVENT);
         * myEvent.setOffset(myEvent.getOffset() - 1);
         * //myEvent.setLength(myEvent.getLength()+1); unbalancedQuote = false;
         * return; } else if ((!EventValue.startsWith("'") && EventValue
         * .endsWith("'"))) { myToken = fInvalidObjectToken;
         * myEvent.setEventType(MQSCCommandEvent.BAD_QUOTE_EVENT);
         * unbalancedQuote = false; return; } } break; case
         * MQSCCommandEvent.PARAMETER_VALUE_EVENT: if (unbalancedQuote) { if
         * (EventValue.startsWith("'") && !EventValue.endsWith("'")) { myToken =
         * fInvalidParmToken;
         * myEvent.setEventType(MQSCCommandEvent.BAD_QUOTE_EVENT);
         * myEvent.setOffset(myEvent.getOffset() - 1); unbalancedQuote = false;
         * //myEvent.setLength(myEvent.getLength()+1); return; } else if
         * ((!EventValue.startsWith("'") && EventValue .endsWith("'"))) {
         * myToken = fInvalidParmToken;
         * myEvent.setEventType(MQSCCommandEvent.BAD_QUOTE_EVENT);
         * unbalancedQuote = false; return; } } break;
         */
